...
from pyspark.sql.functions import when, lit, pow, log10, hour

# ---------------------------------------------------------------------------
# Étape 1 : Configuration de Spark
# ---------------------------------------------------------------------------
# TODO : Modifier "tp-esme" par le nom de votre répertoire GitHub ou chemin local
os.environ["SPARK_HOME"] = "/workspaces/project-esme/spark-3.2.3-bin-hadoop2.7"
...
processed_df = processed_df.withColumn(
    "time_of_day",
    when((hour(col("date")) >= 6) & (hour(col("date")) < 12), lit("Matin"))
    .when((hour(col("date")) >= 12) & (hour(col("date")) < 18), lit("Après-midi"))
    .when((hour(col("date")) >= 18) & (hour(col("date")) < 24), lit("Soirée"))
    .otherwise(lit("Nuit"))
)
